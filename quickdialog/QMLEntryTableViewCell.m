...
#import "QMLEntryTableViewCell.h"
#import "QuickDialog.h"

@interface QMLEntryTableViewCell ()
- (void)handleActionBarPreviousNext:(UISegmentedControl *)control;
@end

@implementation QMLEntryTableViewCell {
...
- (CGRect)calculateFrameForEntryElement {
    if (_entryElement.title == NULL && _entryElement.image==NULL) {
        return CGRectMake(10,10,self.contentView.frame.size.width-10, self.frame.size.height-20);
    }
    if (_entryElement.title == NULL && _entryElement.image!=NULL){
        self.imageView.image = _entryElement.image;
        [self.imageView sizeToFit];
        return CGRectMake( self.imageView.frame.size.width+10, 10, self.contentView.frame.size.width-10-self.imageView.frame.size.width , self.frame.size.height-20);
    }
    ...
    if (CGRectEqualToRect(CGRectZero, _entryElement.parentSection.entryPosition)) {
        ...
        _entryElement.parentSection.entryPosition = CGRectMake(titleWidth+20,10,totalWidth-titleWidth-_entryElement.appearance.cellBorderWidth, self.frame.size.height-20);
    }
...
- (void)prepareForElement:(QMLEntryElement *)element inTableView:(QuickDialogTableView *)tableView{
...
- (void)textViewDidChange:(UITextView *)textView {
   _entryElement.textValue = _textView.text;
...
- (void)textViewDidBeginEditing:(UITextView *)textView {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        [_quickformTableView scrollToRowAtIndexPath:[_entryElement getIndexPath] atScrollPosition:UITableViewScrollPositionTop animated:YES];
    });


    if (_textView.returnKeyType == UIReturnKeyDefault) {
        ...
        _textView.returnKeyType = returnType;
    }
